...
def get_random() -> float:
    """
    Fetches a random decimal number between 0 and 1 with two decimal places from random.org.

    Returns:
        float: The random number fetched from random.org

    Raises:
        RuntimeError: If the request to random.org fails or encounters a timeout.
